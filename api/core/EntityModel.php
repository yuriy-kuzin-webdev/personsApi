...
class EntityModel extends DataBaseHandler
{
    private $tableName;
    ...
    * @return   string  запрос может быть дополнен конструкцией WHERE
     ...
     * Like part
     * 
     * генерирует часть лайк запроса с псевдопеременными
     * 
     * @param   array
     */
    private function likeBuilder(array $columns) : string
    {
        if ($columns !== [])
        {
            $where = ' WHERE';
     ...
     * @param   array   Ассоциативный массив 
     *                  ["ColumnName"=>"ColumnValue"]
     * @return  int     если успешно возвращает последнюю запись
     *                  -1 в случае провала
     */
    ...
    /**
     * Search in defined columns by keywords match
     * 
     * @param   string  coma separated columns or '*'
     * 
     * @param   array   ["ColumnName"=>"Keywords"]
     *                  синтаксис значений корректируется 
     *                  вне функции
     * 
     * @return  array   Выборка из БД
     */
    public function search(string $columns = '*', array $where = []) : array
    {
        $stmt = $this->pdo->prepare( 
            $this->selectBuilder($columns) 
            . $this->likeBuilder(array_keys($where)) 
        );
        $stmt->execute($where);
        return $stmt->fetchAll();
    }
    //Пример:
    //$entity->search('*',["FName" => "%To%", "LName" => "%To%"])
